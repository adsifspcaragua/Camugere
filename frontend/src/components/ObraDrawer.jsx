...
export default function ObraDrawer({
  ...
  const buscarDadosPorIsbn = async () => {
    ...
    try {
      ...
      const localEditora = [data.location, data.publisher]
        .filter(Boolean)
        .join(" - ")
      ...
      if (data.subjects && data.subjects.length > 0) {
        ...
        const numEncontrado = Object.keys(dicionarioCdd).find(
          (key) =>
            dicionarioCdd[key].toLowerCase() === data.subjects[0].toLowerCase(),
        )
  ...
  return (
    <>
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label={isEdit ? "Editar Obra" : "Nova Obra"}
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-surface-200 px-6 py-5 dark:border-surface-800">
          ...
          <button
            onClick={onClose}
            className="rounded-2xl p-3 text-surface-400 transition-colors hover:bg-surface-100 hover:text-surface-600 dark:text-surface-500 dark:hover:bg-surface-800"
            aria-label="Fechar"
          >
            <X size={22} />
          </button>
        ...
        >
          <div className="flex-1 space-y-5 overflow-y-auto px-6 py-6 scrollbar-thin">
            ...
            <div className="flex gap-4">
              ...
              <div className="flex-1">
                ...
                <input
                  ...
                  onChange={(e) => setLocalPublicacao(e.target.value)}
            ...
            <div className="flex gap-4">
              ...
              <div className="flex-1">
                ...
                <input
                  type="text"
                  value={cddDescricao}
                  onChange={handleCddDescricaoChange}
            ...
            {!isEdit && (
              <div>
                <label className="mb-2 flex items-center gap-2 text-base font-medium text-surface-700 dark:text-surface-300">
                  <Layers size={18} className="text-surface-400" /> Quantidade
                  de Exemplares
                </label>
                <div className="flex items-center gap-3">
                  <button
                    type="button"
                    onClick={() => setNumExemplares((v) => Math.max(1, v - 1))}
                  ...
                  <span className="w-10 text-center text-xl font-bold text-surface-900 dark:text-white">
                    {numExemplares}
                  </span>
                  <button
                    type="button"
                    onClick={() => setNumExemplares((v) => Math.min(20, v + 1))}
                    className="flex h-10 w-10 items-center justify-center rounded-xl border border-surface-200 text-lg font-semibold text-surface-600 hover:bg-surface-100 dark:border-surface-700 dark:text-white dark:hover:bg-surface-800"
                  >
                    +
                  </button>
